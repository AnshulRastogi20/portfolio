...
const VideoEditing = () => {
  ...
  const { videoProjects } = useProjects();

  const sortedProjects = [...videoProjects].sort((a, b) => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-light to-white dark:from-dark dark:to-dark/50 pt-20">
      ...
      >
        <h1 className="text-4xl md:text-6xl font-bold text-primary dark:text-white mb-8">Video Editing Projects</h1>
        ...
        <div className="grid gap-8">
          {sortedProjects.map((project) => (
            <motion.div
              ...
              transition={{ duration: 0.5 }}
            >
              <Link to={`/projects/video/${project.title.toLowerCase().replace(/\s+/g, '-')}`}>
